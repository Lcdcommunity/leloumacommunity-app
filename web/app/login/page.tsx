...
export default function LoginPage() {
  ...
  return (
    <>
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=Cormorant+Garamond:wght@400;500;600;700&family=DM+Sans:wght@400;500;700&family=Inter:wght@400;500;700&family=Montserrat:wght@400;500;700&display=swap');
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .lp-orb { position: absolute; border-radius: 50%; filter: blur(80px); pointer-events: none; z-index: 0; opacity: 0.5; }
        .lp-orb-1 { width: 450px; height: 450px; background: radial-gradient(circle, ${getLightColor(theme.primary, 0.25)} 0%, transparent 70%); top: -150px; right: -100px; animation: drift 15s ease-in-out infinite alternate; }
        .lp-orb-2 { width: 350px; height: 350px; background: radial-gradient(circle, ${getLightColor(theme.secondary, 0.15)} 0%, transparent 70%); bottom: -100px; left: -50px; animation: drift 20s ease-in-out infinite alternate-reverse; }

        @keyframes drift { from { transform: translate(0, 0) scale(1); } to { transform: translate(30px, 50px) scale(1.1); } }
        ...
        .lp-card {
          ...
          box-shadow: 0 25px 50px -12px ${getLightColor(theme.secondary, 0.15)}, 0 0 0 1px rgba(255, 255, 255, 0.7) inset;
          opacity: 0; transform: translateY(20px); transition: all 0.8s cubic-bezier(0.22, 1, 0.36, 1);
        }
        .lp-card.visible { opacity: 1; transform: translateY(0); }

        /* ── Dropdown Langue Corner ── */
        .lp-lang-corner {
          position: absolute;
          top: 1.5rem;
        ...
        .lp-lang-select {
          ...
          background-position: ${isRTL ? '0.6rem center' : 'calc(100% - 0.6rem) center'};
          ${isRTL ? 'padding: 0.4rem 0.6rem 0.4rem 1.8rem;' : ''}
        }
        ...
        .lp-logo-box {
          ...
          border-radius: 50%; box-shadow: 0 10px 20px ${getLightColor(theme.secondary, 0.2)};
        }
        .lp-logo-inner { width: 100%; height: 100%; background: white; border-radius: 50%; overflow: hidden; display: flex; align-items: center; justify-content: center; position: relative; }

        .lp-title { font-family: 'Cormorant Garamond', serif; font-size: clamp(1.4rem, 4vw, 1.8rem); font-weight: 700; color: var(--text-deep); text-align: center; line-height: 1.2; }
        .lp-title span { display: block; color: var(--primary); font-size: 0.85em; margin-top: 0.4rem; font-weight: 500; font-family: var(--font-main); }

        .lp-field { margin-bottom: 1.25rem; }
        .lp-label { display: block; font-size: 0.72rem; font-weight: 700; text-transform: uppercase; letter-spacing: 0.08em; color: var(--secondary); margin-bottom: 0.5rem; text-align: ${isRTL ? 'right' : 'left'}; }
        
        .lp-input-wrap { position: relative; }
        .lp-input {
          width: 100%; height: 54px; background: rgba(255, 255, 255, 0.9);
          border: 1.5px solid ${getLightColor(theme.primary, 0.1)};
          border-radius: 14px; padding: 0 1.25rem; font-family: var(--font-main); font-size: 0.95rem; color: var(--text-deep);
          transition: all 0.25s ease; outline: none;
          text-align: ${isRTL ? 'right' : 'left'};
          direction: ${isRTL ? 'rtl' : 'ltr'};
        }
        .lp-input:focus { border-color: var(--primary); box-shadow: 0 0 0 4px ${getLightColor(theme.primary, 0.15)}; background: white; }
        ...
        .lp-btn {
          width: 100%; height: 54px; background: var(--secondary);
          color: white; border: none; border-radius: 14px;
          font-family: var(--font-main); font-size: 1rem; font-weight: 700;
          letter-spacing: 0.03em; cursor: pointer; margin-top: 1.5rem;
        ...
        .lp-btn:hover:not(:disabled) { background: var(--primary); transform: translateY(-2px); box-shadow: 0 15px 30px ${getLightColor(theme.secondary, 0.35)}; }
        .lp-btn:disabled { opacity: 0.7; cursor: not-allowed; }

        .lp-error { background: #FEF2F2; border: 1px solid #FECACA; padding: 0.8rem 1rem; color: var(--error); font-size: 0.82rem; border-radius: 12px; margin-bottom: 1.5rem; display: flex; align-items: center; gap: 0.6rem; }

        .lp-footer { margin-top: 2rem; padding-top: 1.5rem; border-top: 1px dotted ${getLightColor(theme.primary, 0.2)}; text-align: center; }
        .lp-footer p { font-size: 0.85rem; color: var(--text-muted); margin-bottom: 0.8rem; }
        .lp-signup-link { display: inline-flex; align-items: center; gap: 0.5rem; color: var(--secondary); font-weight: 700; text-decoration: none; font-size: 0.9rem; transition: transform 0.2s ease; flex-direction: ${isRTL ? 'row-reverse' : 'row'}; }
        .lp-signup-link:hover { transform: translateX(${isRTL ? '-3px' : '3px'}); color: var(--primary); }
      ...
      <div className="lp-root" dir={isRTL ? 'rtl' : 'ltr'}>
        ...
        <div className={`lp-card ${mounted ? 'visible' : ''}`}>
          ...
          {/* Language Selector Dropdown in Corner */}
          ...
          <div className="lp-logo-wrap">
            ...
            <h1 className="lp-title">
              {t('login.welcome', 'Bienvenue dans votre')} <span>{t('login.secureSpace', 'Espace sécurisé')}</span>
          ...
          <footer className="lp-footer">
            ...
            <Link href="/signup" className="lp-signup-link">
              ...
              <svg width="18" height="18" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24" style={{ transform: isRTL ? 'rotate(180deg)' : 'none' }}>
