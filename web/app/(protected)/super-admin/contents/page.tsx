...
function DetailModal({ 
  ...
  if (content.attachments && content.attachments.length > 0) {
    content.attachments.forEach(att => {
      if (att.url && !allImages.includes(att.url)) allImages.push(att.url);
    });
  }
        ...
        <div style={{ padding:'1rem', borderTop:'1px solid rgba(37,99,235,0.09)', background:'white', borderRadius:'0 0 22px 22px', display:'flex', justifyContent:'center', flexWrap:'wrap', gap:'0.45rem' }}>
          ...
          >
            {busy ? (
              ...
              <svg width="15" height="15" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2.2"><path strokeLinecap="round" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"/></svg>
            )}
...
export default function AdminContentsPage() {
  const [items,        setItems]        = useState<ExtendedContentPost[]>([]);
  ...
  const [detailContent, setDetailContent] = useState<ExtendedContentPost | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<ExtendedContentPost | null>(null);
  ...
  async function handleDelete(c: ExtendedContentPost) {
    setBusyId(c.id);
    ...
    <AppShell title="Informations / contenus">
      <style>{`
        ...
        .ac-card {
          ...
          box-shadow: 0 4px 20px rgba(37, 99, 235, 0.04), 0 0 0 1px rgba(255, 255, 255, 0.9) inset;
          animation: acin 0.4s cubic-bezier(.22,1,.36,1) both;
        }

        .ac-card:hover {
          transform: translateY(-4px);
          background: rgba(255, 255, 255, 0.9);
          box-shadow: 0 12px 30px rgba(37, 99, 235, 0.1), 0 0 0 1px rgba(255, 255, 255, 1) inset;
