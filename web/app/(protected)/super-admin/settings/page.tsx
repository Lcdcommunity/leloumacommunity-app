...
/* ══════════════════════════════════════════════════════ FIELD COMPONENT */
function Field({
  label, value, onChange, placeholder, required = false, mono = false, hint, type = 'text', step, disabled = false, autoComplete, isRTL = false
}: {
  label: string; value: string; onChange: (v: string) => void;
  placeholder?: string; required?: boolean; mono?: boolean; hint?: string; type?: string; step?: string; disabled?: boolean; autoComplete?: string; isRTL?: boolean;
}) {
  ...
  return (
    <div>
      <label style={{ display: 'block', fontSize: '.72rem', fontWeight: 900, color: '#374151', letterSpacing: '.07em', textTransform: 'uppercase', marginBottom: '.45rem' }}>
        {label}{required && <span style={{ color: '#DC2626', [isRTL ? 'marginRight' : 'marginLeft']: 3 }}>*</span>}
      </label>
...
export default function SuperAdminSettingsPage() {
  ...
  useEffect(() => {
    ...
    if ('serviceWorker' in navigator && 'PushManager' in window) {
      navigator.serviceWorker.ready.then(registration => {
        registration.pushManager.getSubscription().then(subscription => {
          if (subscription && isSubscribed) setPushNotifications(true);
        });
      });
    }
    ...
    void (async () => {
      try {
        const [a, pricingData, prefs] = await Promise.all([
          api.getAssociation(),
          ...
          api.getMemberPreferences().catch(() => null), // 🔥 AJOUT CHIRURGICAL : Récupération des préférences
        ]);

        if (!isSubscribed) return;

        // 🔥 INITIALISATION DES PRÉFÉRENCES SAUVEGARDÉES
        if (prefs) {
        ...
        if (isSubscribed) setMsg({ type: 'error', text: t('settings.loadError', 'Erreur de chargement') });
      } finally {
        if (isSubscribed) setInitLoad(false);
      }
    })();
    ...
    if (enable) {
      try {
        ...
        if (subJson.endpoint && subJson.keys?.p256dh && subJson.keys?.auth) {
          ...
          setPrefMsg({ type: 'success', text: t('settings.pushEnabled', 'Notifications push activées !') });
        }
      } catch (error) {
        setPushNotifications(false); 
        setPrefMsg({ type: 'error', text: error instanceof Error ? error.message : "Erreur d'activation" });
      }
    } else {
       try {
         ...
         setPrefMsg({ type: 'success', text: t('settings.pushDisabled', 'Notifications push désactivées.') });
  ...
  async function handlePreferencesSubmit(e: FormEvent) {
    ...
    try {
      ...
      setPrefMsg({ type: 'success', text: t('settings.prefSaveSuccess', 'Préférences enregistrées avec succès.') });
    } catch {
      setPrefMsg({ type: 'error', text: t('settings.prefSaveError', 'Erreur lors de l\'enregistrement des préférences.') });
    } finally {
    ...
    <AppShell title={t('settings.superAdminTitle', 'Paramètres Généraux')}>
      <div dir={isRTL ? 'rtl' : 'ltr'}>
        <style>{`
          ...
          .ss-switch-thumb { position: absolute; top: 3px; left: ${isRTL ? 'auto' : '3px'}; right: ${isRTL ? '3px' : 'auto'}; width: 18px; height: 18px; border-radius: 50%; background: white; transition: transform 0.2s cubic-bezier(.22,1,.36,1); box-shadow: 0 1px 4px rgba(0,0,0,.15); pointer-events: none; }
          .ss-switch input:checked ~ .ss-switch-thumb { transform: translateX(${isRTL ? '-18px' : '18px'}); }
        ...
        <div className="ss-wrap">
          ...
          <div className="ss-header">
            <div className="ss-eyebrow"><div className="ss-dot" />{t('settings.superAdmin', 'Super Admin')}</div>
            <h1 className="ss-title">{t('settings.assocSettings', 'Paramètres de')} <span>{t('settings.assocSettingsHighlight', "l'association")}</span></h1>
          </div>
          ...
          <div className="ss-layout">
              ...
              <div className="ss-panel ss-panel-left">
                <div className="ss-panel-head">
                  <div className="ss-panel-head-left">
                    ...
                    <span className="ss-panel-title">{t('settings.configAssoc', "Configuration de l'association")}</span>
                  </div>
                  {!isEditing && !initLoad && (
                  ...
                  <form onSubmit={(e: FormEvent<HTMLFormElement>) => void onSubmit(e)}>
                    <div className="ss-panel-body" style={{ paddingBottom: isEditing ? '1rem' : '1.5rem' }}>
                      <div className="ss-form-stack">
                        ...
                        <Field
                          label={t('settings.orgName', "Nom officiel de l'organisation")}
                          value={name}
                          onChange={setName}
                        ...
                        <Field
                          label={t('settings.uniqueId', 'Identifiant unique (Code)')}
                          value={code}
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '1.25rem' }}>
                          <Field
                            label={`${t('settings.cotisation', 'Cotisation')} (${activeCurrency})`}
                            ...
                            step="0.01"
                            disabled={!isEditing}
                            hint={t('settings.monthlyPriceHint', 'Prix de base pour un mois.')}
                            isRTL={isRTL}
                          />
                          <Field
                            label={`${t('settings.memberCard', 'Carte membre')} (${activeCurrency})`}
                            ...
                            step="0.01"
                            disabled={!isEditing}
                            hint={t('settings.annualCardHint', 'Prix de la carte annuelle.')}
                            isRTL={isRTL}
                          />
                          <Field
                            label={`${t('settings.expenseThreshold', 'Seuil dépenses')} (${activeCurrency})`}
                ...
                <div className="ss-panel ss-panel-left" style={{ animationDelay: '.12s' }}>
                  <div className="ss-panel-head">
                    <div className="ss-panel-head-left">
                      ...
                      <span className="ss-panel-title">{t('settings.preferencesTitle', 'Paramètres & préférences')}</span>
                    </div>
                  </div>

                  <div className="ss-section">
                    <div className="ss-section-label">{t('settings.notificationChannels', 'Canaux de notification')}</div>
                    {[
                      {
                  ...
                  <div className="ss-section">
                    <div className="ss-section-label">{t('settings.languageDisplay', 'Langue & affichage')}</div>
                    <div className="ss-field-group">
                      <label className="ss-field-label">{t('settings.interfaceLanguage', "Langue de l'interface")}</label>
                    ...
                    <div className="ss-field-group">
                      ...
                      <div className="ss-theme-row">
                        {([
                          ...
                          { value: 'dark',  label: t('settings.themeDark', 'Sombre'), icon: <svg width="14" height="14" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"><path strokeLinecap="round" d="M21 12.79A9 9 0 1111.21 3 7 7 0 0021 12.79z"/></svg> },                          { value: 'system',label: t('settings.themeSystem', 'Système'), icon: <svg width="14" height="14" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"><rect x="2" y="3" width="20" height="14" rx="2" ry="2"/><line x1="8" y1="21" x2="16" y2="21"/><line x1="12" y1="17" x2="12" y2="21"/></svg> },
                        ] as const).map(tPill => (
                          <button
                            key={tPill.value} type="button"
                            className={`ss-theme-pill${mounted && theme === tPill.value ? ' active' : ''}`}
              ...
              <div className="ss-panel ss-panel-left" style={{ animationDelay: '.18s' }}>
                <div className="ss-panel-head">
                  <div className="ss-panel-head-left">
                    ...
                    <span className="ss-panel-title">{t('settings.securityTitle', 'Sécurité du compte')}</span>
                  </div>
                  {!isEditingPassword && (
            ...
            <div className="ss-panel ss-panel-right">
              <div className="ss-panel-head">
                ...
                <span className="ss-panel-title">{t('settings.systemDetails', 'Détails système')}</span>
              </div>
              <div className="ss-panel-body">
                <div className="ss-info-stack">
                  <InfoRow label={t('settings.systemId', 'ID Système')}           value={association?.id ?? '—'}                          mono />
                  <InfoRow label={t('settings.currentCode', 'Code actuel')}       value={association?.code ?? '—'}                        mono />
                  <InfoRow label={t('settings.createdAt', 'Date de création')}    value={association ? formatDate(association.createdAt) : '—'} />
                  <div style={{ borderBottom: 'none' }}>
                    <InfoRow label={t('settings.updatedAt', 'Dernière modification')} value={association ? formatDate(association.updatedAt) : '—'} />
                  </div>
                </div>
                ...
                <div className="ss-governance">
                  ...
                  <div>
                    <div className="ss-gov-title">{t('settings.govRule', 'Règle de gouvernance')}</div>
                    <div className="ss-gov-body">
                      {t('settings.govDesc', "Ces paramètres impactent l'ensemble des antennes et de leurs membres. Seul le profil Super Admin dispose des privilèges nécessaires pour modifier ces informations.")}
                    </div>
                  </div>
                </div>
              </div>
