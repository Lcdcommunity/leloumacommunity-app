...
function StatCard({
  ...
  return (
    <div style={{
      background: 'rgba(253,253,255,.93)', borderRadius: 14,
      border: '1px solid rgba(220,38,38,.09)', borderTop: `3px solid ${color}`,
      boxShadow: '0 2px 8px rgba(220,38,38,.04)', padding: '.8rem 1rem',
      display: 'flex', alignItems: 'flex-start', justifyItems: 'space-between', gap: '.5rem',
    }}>
      <div style={{flex: 1}}>
        <div style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: '1.55rem', fontWeight: 700, color, lineHeight: 1, marginBottom: '.22rem' }}>{value}</div>
        <div style={{ fontSize: '.64rem', fontWeight: 900, color: '#6B7280', textTransform: 'uppercase', letterSpacing: '.07em' }}>{label}</div>
      </div>
...
export default function SuperAdminContributionsPage() {
  ...
  const load = useCallback(async (statusVal?: string) => {
  ...
  const pendingAmt = items
    .filter(c => c.status === 'PENDING' || c.status === 'PENDING_VALIDATION')
    .reduce((s, c) => s + c.amount, 0);
  ...
  return (
    <AppShell title="Cotisations globales">
      <style>{`
        ...
        .sc-urgent-text strong{font-size:.85rem;font-weight:800;color:#111827;display:block;margin-bottom:.1rem}
        ...
        /* Amount summary bar */
        .sc-amounts{display:flex;gap:1.5rem;flex-wrap:wrap;padding:.75rem 1.4rem;border-bottom:1px solid rgba(220,38,38,.06);background:rgba(253,253,255,.6)}
        .sc-amount-item{display:flex;flex-direction:column;gap:.15rem}
      ...
      <div className="sc-wrap">
        ...
        <div className="sc-stats">
          <StatCard
            label="Total cotisations" value={total} color="#DC2626"
          ...
          />
          <StatCard
            label="Valid&eacute;es" value={validated} color="#059669"
        ...
        {hasPending && (
          <div className="sc-urgent">
            ...
            <div className="sc-urgent-text">
              <strong>
                {pending} cotisation{pending > 1 ? 's' : ''} en attente
                {pendingAmt > 0 && <> &mdash; {formatCurrency(pendingAmt, 'GNF')} &agrave; traiter</>}
              </strong>
              <span>Ces cotisations n&apos;ont pas encore &eacute;t&eacute; valid&eacute;es par les administrateurs d&apos;antenne.</span>
            </div>
          </div>
        )}
        ...
        <div className="sc-panel">
          <div className="sc-panel-head">
            <div className="sc-panel-titlerow">
              ...
              <span className="sc-panel-title">Suivi des cotisations &mdash; toutes antennes</span>
          ...
          <div className="sc-toolbar">
            <div className="sc-field">
              ...
              >
                ...
                <option value="REJECTED">Rejet&eacute;es</option>
            ...
            >
              {loading
                ...
                : <><svg width="12" height="12" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2.5"><polygon points="22 3 2 3 10 12.46 10 19 14 21 14 12.46 22 3"/></svg>Filtrer</>
              }
            </button>
          </div>
              ...
              {(Object.entries(STATUS_MAP) as [ContributionStatus, typeof STATUS_MAP[ContributionStatus]][]).map(([key, s]) => {
                ...
                return (
                  <button
                    ...
                    onClick={() => { setStatus(key); void load(key); }}
                  >
                    <span style={{ width: 5, height: 5, borderRadius: '50%', background: s.color, flexShrink: 0 }} />
                    {s.label}
                    <span style={{ fontFamily: "'DM Mono',monospace", fontSize: '.68rem', fontWeight: 700, marginLeft: '.15rem' }}>{count}</span>
                  </button>
                );
              })}
              {status && (
          ...
          {!loading && items.length > 0 && (
            <div className="sc-amounts">
              <div className="sc-amount-item">
                <span className="sc-amount-lbl">Total valid&eacute;</span>
                <span className="sc-amount-val" style={{ color: '#059669' }}>{formatCurrency(totalAmt, 'GNF')}</span>
              </div>
              {pendingAmt > 0 && (
                <div className="sc-amount-item">
                  <span className="sc-amount-lbl">En attente</span>
                  <span className="sc-amount-val" style={{ color: '#D97706' }}>{formatCurrency(pendingAmt, 'GNF')}</span>
                </div>
              )}
              {rejected > 0 && (
                <div className="sc-amount-item">
                  <span className="sc-amount-lbl">Rejet&eacute;es</span>
                  <span className="sc-amount-val" style={{ color: '#DC2626' }}>{rejected}</span>
                </div>
              )}
            </div>
          )}
          ...
          {loading ? (
            ...
            <div className="sc-empty">
              ...
              <div className="sc-empty-title">Aucune cotisation trouv&eacute;e</div>
