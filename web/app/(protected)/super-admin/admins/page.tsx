...
export default function SuperAdminAdminsPage() {
  ...
  const load = useCallback(async (qVal?: string) => {
    ...
    setError(null);
    try {
      const res = await superAdminApi.listAntennaAdmins({ page: 1, pageSize: 100, q: qVal ?? q });
      setItems(res.items);
    } catch (err) {
  ...
  const pendingCount = items.filter((u) => u.status === 'PENDING_APPROVAL').length; // NOUVEAU: Comptes en attente
  ...
  return (
    <AppShell title="Admins d'antenne">
      <style>{`
        ...
        /* ── Header révisé pour inclure le bouton ── */
        .sad-header {
          display: flex; justify-content: space-between; align-items: flex-end;
          margin-bottom: 1.5rem; flex-wrap: wrap; gap: 1rem;
          opacity: 0; transform: translateY(10px); animation: sadin .5s .04s cubic-bezier(.22,1,.36,1) forwards;
        }
        .sad-header-text { display: flex; flex-direction: column; }
        ...
        .sad-title { font-family: 'Cormorant Garamond', serif; font-size: clamp(1.5rem, 3vw, 2rem); font-weight: 700; color: #111827; letter-spacing: -.02em; line-height: 1.15; margin: 0; }
        ...
        .sad-new-btn { height: 40px; padding: 0 1.2rem; border-radius: 11px; background: linear-gradient(135deg,#991B1B,#DC2626); border: none; color: white; cursor: pointer; font-family: 'DM Sans', sans-serif; font-size: .8rem; font-weight: 800; display: flex; align-items: center; gap: .4rem; box-shadow: 0 4px 14px rgba(220,38,38,.32); transition: all .18s; text-decoration: none; white-space: nowrap; flex-shrink: 0; }
        .sad-new-btn:hover { transform: translateY(-1px); box-shadow: 0 6px 20px rgba(220,38,38,.42); }

        /* ── Stats sur UNE SEULE ligne (4 cartes) ── */
        .sad-stats {
          display: grid;
          grid-template-columns: repeat(4, 1fr);
          gap: 0.7rem; margin-bottom: 1.4rem;
          opacity: 0; transform: translateY(10px); animation: sadin .5s .08s cubic-bezier(.22,1,.36,1) forwards;
        }
        .sad-stat { display: flex; flex-direction: column; align-items: center; justify-content: center; text-align: center; background: rgba(253,253,255,.93); border-radius: 14px; border: 1px solid rgba(220,38,38,.09); border-top: 3px solid; box-shadow: 0 2px 8px rgba(220,38,38,.04); padding: .8rem .5rem; }
        .sad-stat-val { font-family: 'Cormorant Garamond', serif; font-size: 1.7rem; font-weight: 700; line-height: 1; margin-bottom: .3rem; }
        .sad-stat-lbl { font-size: .65rem; font-weight: 800; color: #6B7280; text-transform: uppercase; letter-spacing: .05em; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; max-width: 100%; }
        ...
        /* ── Toolbar (Recherche) sur UNE ligne ── */
        .sad-toolbar { padding: .9rem 1.4rem; border-bottom: 1px solid rgba(220,38,38,.07); width: 100%; box-sizing: border-box; }
        .sad-search-row { display: flex; align-items: center; gap: .65rem; width: 100%; flex-wrap: nowrap; }
        
        .sad-sw { position: relative; flex: 1 1 auto; min-width: 0; }
        .sad-si { position: absolute; left: .8rem; top: 50%; transform: translateY(-50%); color: #9CA3AF; pointer-events: none; }
        .sad-search { width: 100%; height: 42px; border-radius: 11px; border: 1px solid rgba(220,38,38,.15); background: rgba(255,255,255,.9); padding: 0 .9rem 0 2.4rem; font-family: 'DM Sans', sans-serif; font-size: .84rem; font-weight: 500; color: #111827; outline: none; transition: border-color .2s, box-shadow .2s; box-sizing: border-box; }
        ...
        .sad-search-btn { flex: 0 0 auto; height: 42px; padding: 0 1.2rem; border-radius: 11px; background: linear-gradient(135deg,#1D4ED8,#2563EB); border: none; color: white; cursor: pointer; font-family: 'DM Sans', sans-serif; font-size: .82rem; font-weight: 800; display: flex; align-items: center; gap: .4rem; box-shadow: 0 3px 10px rgba(37,99,235,.3); transition: all .18s; white-space: nowrap; }
        ...
        @media (max-width: 640px) {
          .sad-header { align-items: center; margin-bottom: 1.25rem; }
          .sad-title { font-size: 1.4rem !important; }
          .sad-new-btn { height: 38px; padding: 0 0.85rem; font-size: 0.75rem; }
          
          /* 4 Stats sur la même ligne (compressées) */
          .sad-stats { gap: 0.4rem; }
          .sad-stat { padding: 0.6rem 0.2rem; border-radius: 10px; }
          .sad-stat-val { font-size: 1.2rem; }
          .sad-stat-lbl { font-size: 0.5rem; }
        }

        @media (max-width: 500px) {
          /* Toolbar sur une ligne */
          .sad-search-row { gap: 0.4rem; }
          .sad-search { height: 38px; font-size: 0.75rem; padding-left: 2rem; }
          .sad-si { left: 0.6rem; width: 14px; height: 14px; }
          .sad-search-btn { height: 38px; padding: 0 0.8rem; font-size: 0.75rem; }
          .btn-text { display: none; } /* On cache "Rechercher" pour que ça tienne */
        }
      ...
      <div className="sad-wrap">
        ...
        <div className="sad-header">
          ...
          <Link href="/super-admin/admins/new" className="sad-new-btn">
            ...
            <span className="btn-text-desktop">Nouvel admin</span>
          </Link>
        </div>
        ...
        <div className="sad-stats">
          {([
            ...
            { label: 'Suspendus', value: suspendedCount, color: '#DC2626' },
            { label: 'En attente', value: pendingCount, color: '#D97706' },
          ] as const).map((s) => (
            <div key={s.label} className="sad-stat" style={{ borderTopColor: s.color }} title={s.label}>
        ...
        <div className="sad-panel">
          <div className="sad-panel-head">
            ...
            {/* Le bouton "Nouvel admin" a été retiré d'ici pour aller dans le header */}
          </div>

          {/* TOOLBAR (RECHERCHE SUR UNE LIGNE) */}
          <div className="sad-toolbar">
            <div className="sad-search-row">
              ...
              <button className="sad-search-btn" disabled={loading} onClick={() => void load(q)}>
                <svg width="14" height="14" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2.5" style={{ display: 'none' }} className="icon-mobile-only">
                  <path strokeLinecap="round" d="M21 21l-4.35-4.35" />
                </svg>
