...
export default function AdminContentsPage() {
  ...
  const Spinner = () => <div style={{ width:13, height:13, border:'2px solid rgba(37,99,235,0.2)', borderTopColor:'#2563EB', borderRadius:'50%', animation:'acspin .7s linear infinite' }} />;
  ...
  return (
    <AppShell title="Informations / contenus">
      <style>{`
        ...
        .ac-layout{display:grid;grid-template-columns:380px 1fr;gap:1.5rem;align-items:start}
        ...
        .ac-table tbody tr{border-bottom:1px solid rgba(37,99,235,.055);transition:background .15s;animation:acin .4s cubic-bezier(.22,1,.36,1) both}
        .ac-table tbody tr:last-child{border-bottom:none}
        .ac-table tbody tr:hover{background:rgba(37,99,235,.025)}
        .ac-table td{padding:.85rem 1.2rem;vertical-align:middle}
        .ac-row-title{font-size:.86rem;font-weight:800;color:#0F172A;margin-bottom:2px}
        .ac-row-excerpt{font-size:.72rem;color:#6B7280;font-weight:500;max-width:220px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
        .ac-row-date{font-size:.7rem;color:#6B7280;font-weight:500}
        .ac-actions{display:flex;gap:.4rem}
        .ac-cycle-btn{height:32px;padding:0 .75rem;border-radius:8px;border:1px solid rgba(37,99,235,.18);background:rgba(239,246,255,.7);color:#1D4ED8;font-family:'DM Sans',sans-serif;font-size:.71rem;font-weight:700;cursor:pointer;display:flex;align-items:center;gap:.3rem;transition:all .18s;white-space:nowrap}
        .ac-cycle-btn:hover:not(:disabled){background:#DBEAFE;border-color:#2563EB;transform:translateY(-1px)}
        .ac-cycle-btn:disabled{opacity:.5;cursor:not-allowed}
        .ac-del-btn{height:32px;padding:0 .75rem;border-radius:8px;border:1.5px solid rgba(220,38,38,.2);background:rgba(254,242,242,.6);color:#DC2626;font-family:'DM Sans',sans-serif;font-size:.71rem;font-weight:700;cursor:pointer;display:flex;align-items:center;gap:.3rem;transition:all .18s;white-space:nowrap}
        .ac-del-btn:hover:not(:disabled){background:#FEE2E2;border-color:rgba(220,38,38,.4);transform:translateY(-1px)}
        .ac-del-btn:disabled{opacity:.5;cursor:not-allowed}
        ...
        .ac-mc{padding:.9rem 1.2rem;border-bottom:1px solid rgba(37,99,235,.07);animation:acin .4s cubic-bezier(.22,1,.36,1) both}
        ...
        .ac-mc-actions{display:flex;gap:.4rem;flex-wrap:wrap;margin-top:.5rem}
          ...
          <div className="ac-panel ac-panel-left">
            <div className="ac-panel-head">
              <div className="ac-panel-title">
                <div className="ac-panel-ico"><svg width="13" height="13" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2.2"><path strokeLinecap="round" strokeLinejoin="round" d="M12 4v16m8-8H4"/></svg></div>
                Publier un contenu
              </div>
            </div>
              ...
              <>
                ...
                <div className="ac-tw">
                  <table className="ac-table">
                    <thead><tr><th>Titre</th><th>Statut</th><th>Date</th><th style={{ textAlign:'right' }}>Actions</th></tr></thead>
                    <tbody>
                      {filtered.map((c, i) => (
                        <tr key={c.id} style={{ animationDelay:`${i * 0.035}s` }}>
                          <td>
                          ...
                          <td>
                            <div className="ac-actions" style={{ justifyContent:'flex-end' }}>
                              <button className="ac-cycle-btn" disabled={busyId===c.id} onClick={() => void handleStatusCycle(c)} title={NEXT_LABEL[c.status]}>
                                {busyId===c.id ? <Spinner /> : <svg width="11" height="11" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2.5"><path strokeLinecap="round" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"/></svg>}
                                {NEXT_LABEL[c.status]}
                              </button>
                              <button className="ac-del-btn" disabled={busyId===c.id} onClick={() => setDeleteTarget(c)}>
                ...
                <div className="ac-mob">
                  {filtered.map((c, i) => (
                    <div key={c.id} className="ac-mc" style={{ animationDelay:`${i * 0.035}s` }}>
                      ...
                      {c.body && <div className="ac-row-excerpt">{c.body}</div>}
                      <div className="ac-mc-actions">
                        <button className="ac-cycle-btn" style={{ flex:1, justifyContent:'center' }} disabled={busyId===c.id} onClick={() => void handleStatusCycle(c)}>
                          {busyId===c.id ? <Spinner /> : <svg width="11" height="11" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2.5"><path strokeLinecap="round" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"/></svg>}
                          {NEXT_LABEL[c.status]}
                        </button>
                        <button className="ac-del-btn" style={{ flex:1, justifyContent:'center' }} disabled={busyId===c.id} onClick={() => setDeleteTarget(c)}>
                          <svg width="11" height="11" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2.5"><path strokeLinecap="round" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg>
                          Supprimer
                        </button>
                      </div>
