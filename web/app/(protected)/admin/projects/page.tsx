  ...
  return (
    <>
      ...
      >
        ...
        >
          <svg width="21" height="21" fill="none" viewBox="0 0 24 24" stroke="#DC2626" strokeWidth="2">
            <path strokeLinecap="round" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
          </svg>
        </div>
        <h2
    ...
    <div>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '.65rem' }}>
        ...
        <span
          style={{
            ...
            background: photos.length > 0 ? '#EFF6FF' : '#F3F4F6',
            border: `1px solid ${photos.length > 0 ? '#BFDBFE' : '#E5E7EB'}`,
            borderRadius: 99,
            padding: '.18rem .55rem',
            fontFamily: "'DM Mono',monospace",
          }}
      ...
      {!full && (
        <div
          ...
          style={{
            border: `2px dashed ${drag ? '#2563EB' : 'rgba(37,99,235,.22)'}`,
            borderRadius: 14,
            padding: '1.4rem 1rem',
            textAlign: 'center',
            cursor: 'pointer',
            background: drag ? 'rgba(239,246,255,.7)' : 'rgba(239,246,255,.3)',
            transition: 'all .18s',
            marginBottom: '.75rem',
            userSelect: 'none',
          }}
        >
          ...
          <p style={{ margin: '0 0 .3rem', fontSize: '.82rem', fontWeight: 800, color: '#1D4ED8' }}>
            Cliquez ou glissez vos photos
          </p>
          <p style={{ margin: 0, fontSize: '.72rem', fontWeight: 600, color: '#9CA3AF' }}>
            PNG, JPG, WEBP — max {MAX_MB} Mo — {remaining} emplacement{remaining > 1 ? 's' : ''} restant
            {remaining > 1 ? 's' : ''}
          </p>
          <input
            ref={inputRef}
...
export default function AdminProjectsPage() {
  ...
  const thStyle: React.CSSProperties = {
    ...
    textAlign: 'left',
    whiteSpace: 'nowrap',
    borderBottom: '2px solid rgba(37,99,235,.08)',
  };
          ...
          <div className="pp-chips">
            {([
              ...
              <div key={c.label} className="pp-chip" style={{ background: c.bg, borderColor: c.border, color: c.color }}>
                ...
                <span style={{ fontSize: '.67rem', opacity: 0.85 }}>{c.label}</span>
              </div>
            ))}
          </div>
