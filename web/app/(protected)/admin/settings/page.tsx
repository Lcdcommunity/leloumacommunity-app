...
export default function AdminSettingsPage() {
  ...
  const togglePushNotifications = async (enable: boolean) => {
    ...
    if (enable) {
      try {
        ...
        if (!vapidPublicKey) {
          throw new Error("Clé VAPID manquante dans la configuration (.env.local).");
        }
        ...
        setPrefMsg({ text: error instanceof Error ? error.message : "Erreur d'activation", ok: false });
      }
    } else {
       try {
         ...
         setPrefMsg({ text: 'Notifications push désactivées.', ok: true });
       } catch(e) {
  ...
  async function handlePreferencesSubmit(e: FormEvent) {
    ...
    try {
      ...
      setPrefMsg({ text: t('settings.saving', 'Préférences enregistrées avec succès.'), ok: true });
    } catch {
