...
export default function AdminSettingsPage() {
  ...
  async function handlePreferencesSubmit(e: FormEvent) {
    ...
    try {
      ...
      setPrefMsg({ text: 'Erreur lors de l\'enregistrement des préférences.', ok: false });
    } finally {
  ...
  return (
    <AppShell title="Paramètres admin">
      <style>{`
        ...
        .ast-switch-thumb { position: absolute; top: 3px; left: 3px; width: 18px; height: 18px; border-radius: 50%; background: white; transition: transform 0.2s cubic-bezier(.22,1,.36,1); box-shadow: 0 1px 4px rgba(0,0,0,0.15); pointer-events: none; }
        .ast-switch input:checked ~ .ast-switch-thumb { transform: translateX(18px); }
        ...
        .ast-select { width: 100%; height: 44px; padding: 0 2.2rem 0 1rem; border-radius: 11px; border: 1px solid rgba(37,99,235,0.15); background: rgba(255,255,255,0.85); font-family: 'DM Sans', sans-serif; font-size: 0.85rem; color: #111827; outline: none; -webkit-appearance: none; appearance: none; cursor: pointer; transition: border-color 0.2s, box-shadow 0.2s; }
        .ast-select:focus { border-color: rgba(37,99,235,0.5); box-shadow: 0 0 0 3px rgba(37,99,235,0.09); background: white; }
        .ast-select-chevron { position: absolute; right: 0.85rem; top: 50%; transform: translateY(-50%); color: #9CA3AF; pointer-events: none; }
            ...
            <form onSubmit={handlePreferencesSubmit}>
              <div className="ast-panel">
                <div className="ast-panel-head">
                  ...
                  <span className="ast-panel-title">Paramètres & préférences</span>
                </div>

                <div className="ast-section">
                  <div className="ast-section-label">Canaux de notification</div>
                  {[
                    {
                ...
                <div className="ast-section">
                  <div className="ast-section-label">Langue &amp; affichage</div>
                  <div className="ast-field-group">
                    <label className="ast-field-label">Langue de l&apos;interface</label>
                  ...
                  <div className="ast-field-group">
                    <label className="ast-field-label">Th&egrave;me</label>
                    <div className="ast-theme-row">
                      {([
                        ...
                        <button
                          ...
                          className={`ast-theme-pill${theme === t.value ? ' active' : ''}`}
                ...
                <div className="ast-form-footer" style={{ justifyContent: 'flex-start' }}>
                  <button type="submit" className="ast-submit-btn" disabled={prefLoading}>
                    {prefLoading ? (
                      <><div style={{ width: 13, height: 13, border: '2px solid rgba(255,255,255,.3)', borderTopColor: 'white', borderRadius: '50%', animation: 'astspin .7s linear infinite' }} />Enregistrement...</>
                    ) : (
