...
function MiniPanel({ title, icon, iconBg, iconColor, count, children, emptyMsg }: {
  ...
  return (
    <div style={{ background:'rgba(253,253,255,.93)', backdropFilter:'blur(12px)', borderRadius:18, border:'1px solid rgba(37,99,235,.09)', boxShadow:'0 2px 14px rgba(37,99,235,.05),0 0 0 1px rgba(255,255,255,.85) inset', overflow:'hidden' }}>
      <div style={{ padding:'.9rem 1.2rem', borderBottom:'1px solid rgba(37,99,235,.07)', display:'flex', alignItems:'center', justifyContent:'space-between', gap:'.75rem' }}>
        <div style={{ display:'flex', alignItems:'center', gap:'.5rem', fontSize:'.72rem', fontWeight:800, letterSpacing:'.08em', textTransform:'uppercase', color:'#1F2937' }}>
            ...
            {dashboardData && (
              <>
                ...
                <div className="aa-grid2">
                  <MiniPanel title="Comptes en attente" icon={<svg width="12" height="12" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2.2"><path strokeLinecap="round" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/></svg>} iconBg="#FFFBEB" iconColor="#D97706" count={dashboardData.recentPendingAccounts.length} emptyMsg="Aucun compte en attente">
                    <table className="aa-mt">
                      <thead><tr><th style={thStyle}>Membre</th><th style={thStyle}>Statut</th><th style={thStyle}>Date</th></tr></thead>
                      <tbody>
                        {dashboardData.recentPendingAccounts.map((u, i) => (
                ...
                <div className="aa-grid2 aa-grid2-b">
                  <MiniPanel title="Projets récents" icon={<svg width="12" height="12" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2.2"><path strokeLinecap="round" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2"/></svg>} iconBg="#ECFDF5" iconColor="#059669" count={dashboardData.recentProjects.length} emptyMsg="Aucun projet">
                    <table className="aa-mt">
                  ...
                  <MiniPanel title="Retardataires +3 mois" icon={<svg width="12" height="12" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2.2"><path strokeLinecap="round" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>} iconBg="#FEF2F2" iconColor="#DC2626" count={dashboardData.lateMembers.length} emptyMsg="Aucun retardataire — bravo !">
                    <table className="aa-mt">
                      <thead><tr><th style={thStyle}>Membre</th><th style={thStyle}>Retard</th><th style={thStyle}>Dernière cotis.</th></tr></thead>
                      <tbody>
                        {dashboardData.lateMembers.map((m, i) => (
