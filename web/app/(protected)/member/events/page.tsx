  ...
  return (
    <div className="mev-modal-overlay" onClick={onClose}>
      <div className="mev-modal" onClick={e => e.stopPropagation()}>
        <div style={{ height: 6, background: isExpired ? '#9CA3AF' : 'linear-gradient(90deg, #059669, #34D399)' }} />
          ...
          {isExpired ? (
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem' }}>
              <button className="mev-btn-yes" onClick={() => handleRSVP('ATTENDING')} disabled={saving}>
                {saving ? 'En cours...' : (
                  <><svg width="18" height="18" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="3"><path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" /></svg> Je participe</>
                )}
              </button>
  ...
  return (
    <AppShell title="Mes Événements">
      <style>{`
        ...
        .mev-card.expired { opacity: 0.65; filter: grayscale(0.5); border-color: #E5E7EB; box-shadow: none; }
        .mev-card.expired:hover { transform: none; border-color: #D1D5DB; }
        ...
        .mev-card.expired .mev-date { color: #6B7280; }
      ...
      {selectedEvent && (
        <MemberEventModal 
          ...
          onSuccess={() => { setSelectedEvent(null); void load(); }} 
        />
      )}
    </AppShell>
  );
